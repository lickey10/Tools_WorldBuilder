...
public class TerrainTexturing
{
    ...
    public static void GenerateTexture(List<TTexture> textures)
    {
        ...
        for (int i = 0; i < textures.Count; i++)
        {
            ...
            terrainLayers[i] = new TerrainLayer() { diffuseTexture = (Texture2D)textures[i].texture, tileSize = textures[i].tilesize };
        }
        ...
        while (x < td.alphamapHeight)
        {
            ...
            while (y < td.alphamapWidth)
            {
                ...
                float xS = x / td.heightmapResolution;
                float yS = y / td.heightmapResolution;
